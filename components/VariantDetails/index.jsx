...
export default function VariantDetails({ data }) {
  const [variantSelected, setVariantSelected] = useState(data[0]);
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      handleCalcProfitMargin(variantSelected?.Cost);
    }, 1200);

    return () => {
      clearTimeout(debounceRef.current);
    };
  }, [variantSelected?.Cost, variantSelected?.price]);
  ...
  const handleCalcProfitMargin = useCallback(
    ...
    [variantSelected?.price, variantSelected?.Cost]
  );

  return (
    <div className="max-w-7xl">
      <div className="my-10 px-2">{` <- ${variantSelected?.title} `}</div>
          ...
          <div className=" bg-white rounded-lg border-[0.5px] shadow sticky top-5 ">
            ...
            <div className="py-5 flex flex-col gap-5  overflow-y-auto h-[calc(100vh-320px)]">
              {data.map((value) => (
                <div
                  className={cn(
                    variantSelected.title == value.title ? "bg-gray-100" : "",
        ...
        <div className="lg:col-span-6 col-span-3  flex flex-col gap-6 ">
          <div className=" bg-white rounded-lg border-[0.5px] shadow p-5">
            ...
            <div className="flex gap-4 mt-5  flex-col">
              {Object.entries(variantSelected.options).map(
                ([key, value], index) => (
                ...
                <div className="grid gap-2 min-w-60 w-[30%]">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVariantSelected(
                        produce((draft) => {
                          draft.Compare = e.target.value;
                        })
                      )
                    }
                    value={variantSelected.Compare}
                    defaultValue={"00.00"}
                  />
                </div>
              </div>
                ...
                <div className="grid gap-2 min-w-60 w-[30%]">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setVariantSelected(
                        produce((draft) => {
                          draft.Cost = e.target.value;
                        })
                      )
                    }
                    // value={variantSelected?.Cost||0}
                    defaultValue={variantSelected?.Cost || 0}
                  />
                </div>
                <div className="grid gap-2 min-w-60 w-[30%]">
